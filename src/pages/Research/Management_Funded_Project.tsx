...
const Management_Funded_Project: React.FC = () => {
    ...
    coInvestigator:
      isMultidisciplinary === "Yes" && activeTab === "2"
        ? Yup.object({
            ...
            fellowshipFile: Yup.mixed().required(
              "Please upload the fellowship file"
            ),
          })
        : Yup.object(),
  });
  ...
  const handleDeleteFile = async () => {
    try {
      // Call the delete API
      const response = await api.delete(
        `/managementFundProject/deleteManagementFundedProjectDocument?managementFundProjectId=${editId}&docType=${documentType}`,
        ""
      );
