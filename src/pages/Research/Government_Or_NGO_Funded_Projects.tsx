...
const GovernmentOrNGOFundedProjects = () => {
      ...
      if (isMultidisciplinary === "Yes") {
        if (activeTab === "1") {
          if (typeof values.principalInvestigator.abstractFile === "string") {
            ...
            values.principalInvestigator.abstractFile instanceof File
          ) {
            formData.append(
              "abstractProject",
          ...
          if (
            ...
            values.principalInvestigator.sanctionOrderFile instanceof File
          ) {
            formData.append(
              "sanctionOrder",
  ...
  const handleDeleteFile = async () => {
    try {
      const response = await api.delete(
        `/governmentFundProject/deleteGovernmentFundedProjectDocument?GovernmentFundProjectId=${editId}&docType=${documentType}`,
        ""
      );
