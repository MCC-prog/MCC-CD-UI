...
const Innovation: React.FC = () => {
  ...
  const [inovData, setInovData] = useState<any[]>([]);
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = inovData.filter((row) =>
  ...
  const dropdownStyles = {
    menu: (provided: any) => ({
      ...provided,
      maxHeight: 200, // or any value you prefer
      overflowY: "auto",
    }),
  };
  ...
  const fetchInovData = async () => {
    try {
      const response = await api.get("/innovation/getAllInnovation", "");
      setInovData(response);
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/innovation?innovationId=${id}`, "");
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error("Failed to remove Innovation. Please try again.");
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        const response = await axios.get(`/innovation/download/${fileName}`, {
          responseType: "blob",
        });
  ...
  const handleDeleteFile = async (fileName: string, docType: string) => {
    try {
      // Call the delete API
      const response = await api.delete(
        `/innovation/deleteInnovationDocument?fileName=${fileName}`,
        ""
      );
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      if (isEditMode && typeof values.file === "string") {
        ...
        formData.append("innovation", values.file);
      }

      try {
        if (isEditMode && editId) {
          // Call the update API
          const response = await api.put(`/innovation`, formData);
          toast.success(response.message || "Innovation updated successfully!");
        } else {
          // Call the save API
          const response = await api.create("/innovation", formData);
          toast.success(response.message || "Innovation added successfully!");
        }
        ...
        toast.error("Failed to save Innovation. Please try again.");
                  ...
                  <Col sm={4}>
                    <div className="mb-3">
                      ...
                      <div className="form-text">
                        Name should be{" "}
                        <b>InnovationEcosystem-Document-School-Dept-Year</b>
                      </div>
