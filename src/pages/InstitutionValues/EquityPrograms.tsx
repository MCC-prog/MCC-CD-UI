...
const EquityPrograms: React.FC = () => {
  ...
  const fetchActivitiesData = async () => {
    try {
      const response = await api.get(
        "/institutionalValues/getAllInstitutionalValues?screenType=equityProgram",
        ""
      );
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(
        `/institutionalValues?institutionalValuesId=${id}&screenType=equityProgram`,
        ""
      );
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        toast.error(
          "Failed to download Equity Program file. Please try again."
        );
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumb
            ...
            breadcrumbItem="Gender Sensation & Equity Programs"
          />
          <Card>
            <CardBody>
