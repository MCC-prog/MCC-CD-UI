...
const BestPractices: React.FC = () => {
  ...
  const fetchActivitiesData = async () => {
    try {
      const response = await api.get("/institutionalValues/getAllInstitutionalValues?screenType=bestPractices", "");
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/institutionalValues?institutionalValuesId=${id}&screenType=bestPractices`, "");
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        toast.error("Failed to download Best Practices file. Please try again.");
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="Institutional Values" breadcrumbItem="Best Practices" />
          <Card>
            <CardBody>
