...
const ConstitutionalValues: React.FC = () => {
  ...
  const fetchActivitiesData = async () => {
    try {
      const response = await api.get(
        "/institutionalValues/getAllInstitutionalValues?screenType=constitutionalValues",
        ""
      );
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(
        `/institutionalValues?institutionalValuesId=${id}&screenType=constitutionalValues`,
        ""
      );
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error(
          "Failed to remove Constitutional Values. Please try again."
        );
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        toast.error(
          "Failed to download Constitutional Values file. Please try again."
        );
      ...
      <div className="page-content">
        <Container fluid>
          <Breadcrumb
            ...
            breadcrumbItem="Constitutional Values"
          />
          <Card>
            <CardBody>
                ...
                {activityData.length > 0 ? (
                  activityData.map((activity, index) => (
                    <tr key={activity.id}>
                      ...
                      <td className="d-none">
                        {activity?.filePath?.constitutionalValues || "N/A"}
                      </td>{" "}
                      {/* Hidden */}
                      <td>
