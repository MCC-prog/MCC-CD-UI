...
const ConstitutionalValues: React.FC = () => {
  ...
  // State variable for managing the modal for listing Constitutional Values
  const [isModalOpen, setIsModalOpen] = useState(false);
  // State variable for managing the list of Constitutional Values data
  ...
  // Toggle the modal for listing Constitutional Values
  const toggleModal = () => {
    setIsModalOpen(!isModalOpen);
  };

  // Fetch Constitutional Values data from the backend
  const fetchActivitiesData = async () => {
    try {
      const response = await api.get("/institutionalValues/getAllInstitutionalValues?screenType=constitutionalValues", "");
      setActivityData(response);
      setFilteredData(response);
    } catch (error) {
      console.error("Error fetching Constitutional Values data:", error);
    }
  };
  ...
  // Handle edit action
  // Fetch the data for the selected Constitutional Values ID and populate the form fields
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/institutionalValues?institutionalValuesId=${id}&screenType=constitutionalValues`, "");
      ...
      console.error("Error fetching Constitutional Values data by ID:", error);
    }
  };
  ...
  // Call the delete API and refresh the Constitutional Values data
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        const response = await api.delete(`/institutionalValues/deleteInstitutionalValues?institutionalValuesId=${id}`, "");
        toast.success(
          response.message || "Constitutional Values removed successfully!"
        );
        fetchActivitiesData();
      } catch (error) {
        toast.error("Failed to remove Constitutional Values. Please try again.");
        console.error("Error deleting Constitutional Values:", error);
      } finally {
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        toast.error("Failed to download Constitutional Values file. Please try again.");
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      formData.append("screenType", "constitutionalValues");
      ...
      try {
        ...
        toast.success(
          response.message || "Constitutional Values updated successfully!"
        );
        ...
        // display the Constitutional Values List
        handleListActivityClick();
      } catch (error) {
        // Display error message
        toast.error("Failed to save Constitutional Values. Please try again.");
        console.error("Error creating Constitutional Values:", error);
      }
    },
  });
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="Institutional Values" breadcrumbItem="Constitutional Values" />
          <Card>
            <CardBody>
        ...
        {/* Modal for Listing Constitutional Values */}
        <Modal
          isOpen={isModalOpen}
          toggle={toggleModal}
          size="lg"
        ...
        >
          <ModalHeader toggle={toggleModal}>List Constitutional Values</ModalHeader>
          <ModalBody>
            {/* Global Search */}
            <div className="mb-3">
              <Input
                ...
                {currentRows.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={11} className="text-center">
                      No Constitutional Values data available.
                    </td>
                  </tr>
                )}
              </tbody>
...
export default ConstitutionalValues;
