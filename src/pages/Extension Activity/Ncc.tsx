...
const Ncc: React.FC = () => {
  ...
  const fetchNccData = async () => {
    try {
      const response = await api.get("/ncc/getAllNcc", "");
      setNccData(response.data);
      console.log("NCC data fetched successfully:", response.data);
    } catch (error) {
  ...
  const handleEdit = (id: string) => {
    console.log("Edit NCC with ID:", id);
    // Add your edit logic here
  };

  const handleDelete = (id: string) => {
    console.log("Delete NCC with ID:", id);
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      // Append the file
      if (values.file) {
        formData.append("ncc", values.file);
      } else {
        console.error("No file selected");
      }

      try {
        const response = await api.create("/ncc/saveNcc", formData);
        // Display success message
        toast.success(response.message || "Ncc added successfully!");
        console.log("Ncc created successfully:", response.data);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="NCC" breadcrumbItem="NCC" />
          <Card>
            <CardBody>
                ...
                {nccData.length > 0 ? (
                  nccData.map((ncc, index) => (
                    <tr key={ncc.nssId}>
                      ...
                      <td>{ncc.programName}</td>
                      <td>{ncc.semType}</td>
                      <td>{ncc.noOfParticipants}</td>
                      <td>{ncc.organization}</td>
                      <td>{ncc.location}</td>
                      <td>{ncc.date}</td>
                      <td>
                        <button
                          className="btn btn-sm btn-warning me-2"
                          onClick={() => handleEdit(ncc.nssId)}
                        >
                          Edit
                        </button>
                        <button
                          className="btn btn-sm btn-danger"
                          onClick={() => handleDelete(ncc.nssId)}
                        >
                          Delete
                        </button>
                      </td>
