...
const Isrc: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState(isrcData);
  ...
  const fetchIsrcData = async () => {
    try {
      const response = await api.get("/ISRC/getAll", "");
      setIsrcData(response);
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/ISRC/edit?isrcId=${id}`, "");
      ...
      const mappedValues = {
        ...
        file: response.documents?.ISRC || null,
      };
      ...
      setIsFileUploadDisabled(!!response.documents?.IRSC);
      toggleModal();
    } catch (error) {
      console.error("Error fetching IRSC data by ID:", error);
    }
  };
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error("Failed to remove ISRC. Please try again.");
        console.error("Error deleting ISRC:", error);
      } finally {
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        const response = await axios.get(`/ISRC/download//${fileName}`, {
          responseType: "blob",
        });
  ...
  const handleDeleteFile = async () => {
    try {
      ...
      const response = await api.delete(
        `/ISRC/deleteISRCDocument?isrcId=${editId}`,
        ""
      );
...
toast.success(response.message || "File deleted successfully!");
  ...
  useEffect(() => {
    if (isrcData.length === 0) return; // wait until data is loaded
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="Extension Activity" breadcrumbItem="ISRC" />
          <Card>
            <CardBody>
