...
const Iks: React.FC = () => {
  ...
  // Fetch IKS data from the backend
  const fetchIksData = async () => {
    try {
      const response = await api.get("/IKS/getAll", "");
      setIksData(response);
      setFilteredData(response);
    } catch (error) {
      console.error("Error fetching IKS:", error);
    }
  };
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/IKS/edit?iksId=${id}`, "");
      ...
      const mappedValues = {
        ...
        file: response.documents?.IKS || null,
      };
      ...
      console.error("Error fetching IKS data by ID:", error);
    }
  };
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error("Failed to remove IKS . Please try again.");
        console.error("Error deleting IKS:", error);
      } finally {
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        const response = await axios.get(`/IKS/download//${fileName}`, {
          responseType: "blob",
        });
  ...
  const handleDeleteFile = async () => {
    try {
      ...
      const response = await api.delete(
        `/IKS/deleteIKSDocument?iksId=${editId}`,
        ""
      );
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (isEditMode && editId) {
          ...
          toast.success(response.message || "IKS Activity added successfully!");
        }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="Extension Activity" breadcrumbItem="IKS" />
          <Card>
            <CardBody>
                ...
                {iksData.length > 0 ? (
                  iksData.map((iks, index) => (
                    <tr key={iks.id}>
                      ...
                      <td>{iks.departmentName}</td>
                      <td>
                        <ul>
                          {(Object.values(iks.courses) as string[]).map(
                            (course, index) => (
