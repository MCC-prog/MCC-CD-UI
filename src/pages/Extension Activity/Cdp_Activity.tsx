...
const Cdp_Activites: React.FC = () => {
  ...
  const fetchCdpData = async () => {
    try {
      ...
      setCdpData(response.data);
      console.log("CDP Activity data fetched successfully:", response.data);
    } catch (error) {
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      // Append the file
      if (values.file) {
        formData.append("cdp", values.file);
      } else {
        console.error("No file selected");
      }

      try {
        const response = await api.create("/cdp/saveCdpActivity", formData);
        // Display success message
        toast.success(response.message || "Cdp Activity added successfully!");
        console.log("CDP created successfully:", response.data);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="CdpActivity" breadcrumbItem="Cdp Activity" />
          <Card>
            <CardBody>
                ...
                {cdpData.length > 0 ? (
                  cdpData.map((cdp, index) => (
                    <tr key={cdp.cdpId}>
                      ...
                      <td>{cdp.programName}</td>
                      <td>{cdp.semType}</td>
                      <td>{cdp.noOfParticipants}</td>
                      <td>{cdp.organization}</td>
                      <td>{cdp.location}</td>
                      <td>{cdp.date}</td>
                      <td>
                        <button
                          className="btn btn-sm btn-warning me-2"
                          onClick={() => handleEdit(cdp.cdpId)}
                        >
                          Edit
                        </button>
                        <button
                          className="btn btn-sm btn-danger"
                          onClick={() => handleDelete(cdp.cdpId)}
                        >
                          Delete
                        </button>
                      </td>
