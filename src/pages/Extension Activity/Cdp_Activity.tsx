...
const Cdp_Activites: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState(cdpData);
  ...
  const fetchCdpData = async () => {
    try {
      const response = await api.get("/CDPActivity/getAll", "");
      setCdpData(response);
      setFilteredData(response);
    } catch (error) {
      console.error("Error fetching CDP Activity:", error);
    }
  };

  // Open the modal and fetch data
  const handleListCdpClick = () => {
    toggleModal();
    fetchCdpData();
  };
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/CDPActivity/edit?cdpId=${id}`, "");
      ...
      console.error("Error fetching CDP data by ID:", error);
    }
  };
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error("Failed to remove CDP . Please try again.");
  ...
  const handleDownloadFile = async (fileName: string) => {
    if (fileName) {
      try {
        ...
        const response = await axios.get(`/CDPActivity/download/${fileName}`, {
          responseType: "blob",
        });
  ...
  const handleDeleteFile = async () => {
    try {
      // Call the delete API
      const response = await api.delete(
        `/CDPActivity/deleteCDPDocument?cdpId=${editId}`,
        ""
      );
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (isEditMode && editId) {
          // Call the update API
          const response = await api.put(`/CDPActivity/update`, formData);
          toast.success(
            response.message || "CDP Activity updated successfully!"
          );
        } else {
          // Call the save API
          const response = await api.create("/CDPActivity/save", formData);
          toast.success(response.message || "CDP Activity added successfully!");
        }
        ...
        toast.error("Failed to save CDP Activity. Please try again.");
        console.error("Error creating CDP Activity:", error);
      }
    },
  });

  useEffect(() => {
    if (cdpData.length === 0) return; // wait until data is loaded
