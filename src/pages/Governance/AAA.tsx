...
const AAA: React.FC = () => {
  ...
  const [aaaData, setAaaData] = useState<any[]>([]);
  ...
  const fetchAAAData = async () => {
    try {
      const response = await axios.get("/governanceAAA/getAllAAA"); // Replace with your backend API endpoint
      setAaaData(response);
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(`/governanceAAA/edit?aaaId=${id}`, "");
      ...
      const mappedValues = {
        ...
        file: response.document?.aaa || null,
      };
      ...
      console.error("Error fetching AAA data by ID:", error);
    }
  };
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error("Failed to remove AAA. Please try again.");
        console.error("Error deleting AAA:", error);
      } finally {
    ...
    if (fileName) {
      try {
        ...
        const response = await axios.get(
          `/governanceAAA/download/${fileName}`,
          {
            responseType: "blob",
          }
        );
  ...
  const handleDeleteFile = async () => {
    try {
      // Call the delete API
      const response = await api.delete(
        `/governanceAAA/deleteAAADocument?aaaId=${editId}`,
        ""
      );
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      if (isEditMode && editId) {
        formData.append("aaaId", editId);
      }
      try {
        if (isEditMode && editId) {
          // Call the update API
          const response = await api.put(`/governanceAAA/update`, formData);
          toast.success(response.message || "AAA updated successfully!");
        } else {
          // Call the save API
          const response = await api.create("/governanceAAA/save", formData);
          toast.success(response.message || "AAA added successfully!");
        }
        ...
        toast.error("Failed to save AAA. Please try again.");
        console.error("Error creating AAA:", error);
      }
    },
  });
                    ...
                    <tr key={aaa.aaaId}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleEdit(aaa.aaaId)}
                        >
                          Edit
                        </button>
                        <button
                          className="btn btn-sm btn-danger"
                          onClick={() => handleDelete(aaa.aaaId)}
                        >
                          Delete
                        </button>
                      </td>
