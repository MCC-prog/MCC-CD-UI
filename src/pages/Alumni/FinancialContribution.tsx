  ...
  // State variables for managing modal, edit mode, and delete confirmation
    ...
    const fetchFinancialFileData = async () => {
      try {
        ...
        if (response && Array.isArray(response) && response.length > 0) {
          ...
          setIsEditMode(!!data.file?.FinancialContribution);
          setIsFileUploadDisabled(!!data.file?.FinancialContribution);
          validation.setFieldValue("financialFile", data.file?.FinancialContribution || null, false);
        } else {
          setIsEditMode(false); // No file, not in edit mode
        }
      } catch (error) {
        toast.error("Failed to fetch Financial file data.");
        setIsEditMode(false);
      }
    };
    // Fetch the Career Fair data when the component mounts
    useEffect(() => {
      fetchFinancialFileData();
    }, []);
    ...
    const validation = useFormik({
      ...
      onSubmit: async (values, { resetForm }) => {
        ...
        if (typeof values.financialFile === "string") {
          formData.append("financialContribution", "null");
                  ...
                  <Row>
                    <Col lg={12}>
                      <div className="mt-3 d-flex justify-content-between">
                        {(!validation.values.financialFile || typeof validation.values.financialFile !== "string") && (
