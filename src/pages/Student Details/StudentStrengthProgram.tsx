  ...
  const fetchBosData = async () => {
    try {
      const response = await axios.get(
        "/studentStrengthProgramWise/getAllTotalStudentStrength"
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(
        `/studentStrengthProgramWise/edit?totalStudentStrengthId=${id}`,
        ""
      );
      ...
      try {
        if (isEditMode && editId) {
          ...
          const response = await api.put(
            `/studentStrengthProgramWise/update`,
            formData
          );
          toast.success(
            response.message ||
          ...
          const response = await api.create(
            "/studentStrengthProgramWise/save",
            formData
          );
          toast.success(
            response.message ||
                  ...
                  <Col sm={4}>
                    <div className="mb-3">
                      <Label htmlFor="formFile" className="form-label">
                        Upload Student Strength Program-Wise
                        <i
                          id="infoIcon"
