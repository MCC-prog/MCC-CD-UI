...
const Amphitheatre: React.FC = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [amphitheatreData, setAmphitheatreData] = useState<any[]>([]);
...
const fileRef = useRef<HTMLInputElement | null>(null);
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = amphitheatreData.filter((row) =>
  ...
  const fetchAmphitheatreData = async () => {
    try {
      ...
      console.error("Error fetching Amphitheatre data:", error);
    }
  };

  // Open the modal and fetch data
  const handleListAmphitheatresClick = () => {
    toggleModal();
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.error("Failed to remove Amphitheatre. Please try again.");
        console.error("Error deleting Amphitheatre:", error);
      } finally {
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        setIsFileUploadDisabled(false); // Enable the file upload button
        handleListAmphitheatresClick();
      } catch (error) {
        // Display error message
        toast.error("Failed to save Amphitheatre. Please try again.");
        console.error("Error creating Amphitheatre:", error);
      }
    },
  });
                  ...
                  <Col lg={12}>
                    <div className="mt-3 d-flex justify-content-between">
                      ...
                      <button
                        ...
                        type="button"
                        onClick={handleListAmphitheatresClick}
                      >
                        List
                      </button>
                    </div>
            ...
            <Table
              ...
              <tbody>
                {currentRows.length > 0 ? (
                  currentRows.map((amphitheatre, index) => (
                  ...
                  <tr>
                    <td colSpan={4} className="text-center">
                      No Amphitheatre data available.
                    </td>
                  </tr>
                )}
              </tbody>
