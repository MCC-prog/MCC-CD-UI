...
const Auditorium: React.FC = () => {
    ...
    const fetchAuditoriumData = async () => {
        try {
            const response = await api.get("/infrastructureAuditorium/getAllAuditorium", "");
    ...
    const handleEdit = async (id: string) => {
        try {
            const response = await api.get(`/infrastructureAuditorium/edit?auditoriumId=${id}`, "");
    ...
    const confirmDelete = async (id: string) => {
        if (deleteId) {
            try {
                const response = await api.delete(`/infrastructureAuditorium/deleteAuditorium?auditoriumId=${id}`, "");
                toast.success(
    ...
    const handleDownloadFile = async (fileName: string) => {
        if (fileName) {
            try {
                ...
                const response = await axios.get(`/infrastructureAuditorium/download/${fileName}`, {
                    responseType: "blob",
                });
            ...
            if (isEditMode && typeof values.file === "string") {
             ...
             try {
                    if (isEditMode && editId) {
                      // Call the update API
                      const response = await api.put(`/infrastructureAuditorium/update`, formData);
                      toast.success(response.message || "Auditorium updated successfully!");
                    } else {
                      // Call the save API
                      const response = await api.create("/infrastructureAuditorium/save", formData);
                                    ...
                                    <Col sm={4}>
                                        <div className="mb-3">
                                            ...
                                            <Input
                                                ...
                                                onChange={e => validation.setFieldValue("seatingCapacity", e.target.value)}
                                                placeholder="Enter seating capacity"
                                                className={validation.touched.seatingCapacity && validation.errors?.seatingCapacity ? "is-invalid" : ""}
                                            />
                                            {validation.touched.seatingCapacity && validation.errors?.seatingCapacity && (
                                                <div className="text-danger">{validation.errors.seatingCapacity}</div>
                                            )}
                                        </div>
                                    </Col>
                                ...
                                <tr>
                                    ...
                                    <th>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => handleFilterChange(e, "seatingCapacity")}
                                        />
                                    </th>
                                    <th>Actions</th>
                                </tr>
