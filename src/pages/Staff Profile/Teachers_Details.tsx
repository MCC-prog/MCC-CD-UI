  ...
  const fetchBosData = async () => {
    try {
      const response = await axios.get(
        "/teacherDetails/getAllClassRoomTools?screenName=TEACHERDETAILS"
  ...
  const confirmDelete = async (id: string) => {
    if (deleteId) {
      try {
        ...
        toast.success(
          response.message || "Teachers Details removed successfully!"
        );
        fetchBosData();
      } catch (error) {
        toast.error("Failed to remove Teachers Details. Please try again.");
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (isEditMode && editId) {
          ...
          toast.success(
            response.message || "Teachers Details updated successfully!"
          );
        } else {
          // Call the save API
          const response = await api.create("/teacherDetails/save", payload);
          toast.success(
            response.message || "Teachers Details added successfully!"
          );
        }
        ...
        toast.error("Failed to save Teachers Details. Please try again.");
