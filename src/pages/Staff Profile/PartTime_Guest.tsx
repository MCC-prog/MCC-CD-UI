  ...
  const fetchBosData = async () => {
    try {
      const response = await axios.get(
        "/teacherDetails/getAllClassRoomTools?screenName=GUESTFACULTY"
  ...
  const handleEdit = async (id: string) => {
    try {
      const response = await api.get(
        `/teacherDetails/edit?id=${id}&screenName=GUESTFACULTY`,
        ""
      );
      if (!response) {
      ...
      validation.setValues({
        ...
        stream: mapValueToLabel(response.streamName, []), // Assuming you have a stream options array
        department: mapValueToLabel(
    ...
    if (deleteId) {
      try {
        const response = await api.delete(
          `/teacherDetails/deleteClassRoomTools?id=${id}&screenName=GUESTFACULTY`,
          ""
        );
        toast.success(
          response.message || "Part-Time/Guest Faculty removed successfully!"
        );
        fetchBosData();
      } catch (error) {
        toast.error(
          "Failed to remove Part-Time/Guest Faculty. Please try again."
        );
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        if (isEditMode && editId) {
          ...
          toast.success(
            response.message || "Part-Time/Guest Faculty updated successfully!"
          );
        } else {
          // Call the save API
          const response = await api.create("/teacherDetails/save", payload);
          toast.success(
            response.message || "Part-Time/Guest Faculty added successfully!"
          );
        }
        ...
        toast.error(
          "Failed to save Part-Time/Guest Faculty. Please try again."
        );
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb
            title="Staff Profile"
            breadcrumbItem="Part-Time/Guest Faculty"
          />
          <Card>
            <CardBody>
